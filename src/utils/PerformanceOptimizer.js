...
class PerformanceOptimizer {
  ...
  adjustSettingsBasedOnMemory(usage) {
    ...
    if (memoryPressure > 0.8) {
      ...
      this.settings.batchSize = Math.max(5, Math.floor(this.settings.batchSize * 0.7));
      this.settings.maxConcurrency = Math.max(1, this.settings.maxConcurrency - 1);
      
      console.log(chalk.yellow("⚠️  High memory usage detected, reducing performance settings"));
    } else if (memoryPressure < 0.5 && this.settings.batchSize < 100) {
      // Low memory pressure - can increase batch size
      this.settings.batchSize = Math.min(100, Math.floor(this.settings.batchSize * 1.1));
    }
  }
  ...
  async processBatch(items, processor, options = {}) {
    ...
    const maxConcurrency = options.maxConcurrency || this.settings.maxConcurrency;
  ...
  async processSingleBatch(batch, processor) {
    ...
    for (const item of batch) {
      try {
        ...
        console.warn(chalk.yellow(`Processing failed for item: ${error.message}`));
  ...
  getAverageMemoryUsage() {
    ...
    return Math.round(total / this.metrics.memoryUsage.length / (1024 * 1024)) + ' MB';
  }

  getAverageProcessingRate() {
    if (this.metrics.processingTimes.length === 0) return null;
    
    const total = this.metrics.processingTimes.reduce((sum, time) => sum + time.rate, 0);
    return Math.round(total / this.metrics.processingTimes.length * 100) / 100 + ' items/sec';
  }

  getRecommendations() {
    const recommendations = [];
    const avgMemoryUsage = this.metrics.memoryUsage.length > 0 ? 
      this.metrics.memoryUsage.reduce((sum, usage) => sum + usage.heapUsed, 0) / this.metrics.memoryUsage.length : 0;
    
    if (avgMemoryUsage > this.settings.memoryThreshold * 0.7) {
      recommendations.push("Consider increasing system memory or reducing batch size");
    }
    
    if (this.settings.maxConcurrency < os.cpus().length) {
      recommendations.push("System has unused CPU capacity, consider enabling more concurrency");
    }
    
    if (this.metrics.processingTimes.length > 10) {
      const recentRate = this.metrics.processingTimes.slice(-5).reduce((sum, time) => sum + time.rate, 0) / 5;
      const oldRate = this.metrics.processingTimes.slice(0, 5).reduce((sum, time) => sum + time.rate, 0) / 5;
      
      if (recentRate < oldRate * 0.8) {
        recommendations.push("Performance degradation detected, consider restarting or reducing workload");
      }
    }
