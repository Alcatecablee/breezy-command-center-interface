...
class ErrorHandler {
  ...
  handleError(error, operation = null, options = {}) {
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      console.error(chalk.gray("Check if the file path is correct and accessible"));
    } else if (error.code === 'EMFILE' || error.code === 'ENFILE') {
      console.error(chalk.red("❌ Too Many Open Files"));
      console.error(chalk.yellow("System file descriptor limit reached"));
      console.error(chalk.gray("Try reducing batch size or closing other applications"));
